...
class GerberCairoContext(GerberContext):
    ...
    def dump(self, filename=None, verbose=False):
        ...
        if is_svg:
            ...
            with open(filename, "w") as f:
                ...
                f.write(self.surface_buffer.read().decode('utf-8'))
                f.flush()
        else:
    ...
    def new_render_layer(self, color=None, mirror=False):
        ...
        matrix = cairo.Matrix(xx = self._xform_matrix.xx,
                              yy = self._xform_matrix.yy,
                              xy = self._xform_matrix.xy,
                              yx = self._xform_matrix.yx,
                              x0 = self._xform_matrix.x0,
                              y0 = self._xform_matrix.y0)
