...
class BoardSettings(Operator):
    ...
    def execute(self, context):
        bpy.ops.fritzing.progress_report("INVOKE_DEFAULT")
        return {"FINISHED"}
...
def register():
    ...
    if len(board_thickness_items) == 0:
        ...
        Scene.board_thickness_setting = EnumProperty(items=board_thickness_items)
    ...
    if len(board_color_items) == 0:
        ...
        Scene.board_color_setting = EnumProperty(items=board_color_items)
    ...
    if len(copper_color_items) == 0:
        ...
        Scene.copper_color_setting = EnumProperty(items=copper_color_items)
    ...
    if len(silk_color_items) == 0:
        ...
        Scene.silk_color_setting = EnumProperty(items=silk_color_items)
    ...
    if len(drill_algorithm_items) == 0:
        ...
        Scene.drill_algorithm_setting = EnumProperty(items=drill_algorithm_items)
