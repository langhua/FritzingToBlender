...
# Ensure the project root is on sys.path so that 'io_fritzing' package
# imports work both as a Blender addon and when run directly via
# VS Code Blender Development extension (runpy.run_path)
_gerber_dir = os.path.abspath(os.path.dirname(__file__))
# io_fritzing/gerber -> io_fritzing -> project root (FritzingToBlender)
_project_root = os.path.abspath(os.path.join(_gerber_dir, '..', '..'))
for _p in (_gerber_dir, _project_root):
    if _p not in sys.path:
        sys.path.insert(0, _p)

import bpy

# Support both package-relative imports (normal Blender addon) and
# direct script execution (VS Code Blender Development extension)
try:
    ...
    # When run directly, use absolute package imports
