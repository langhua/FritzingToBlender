...
# 定义 1W 碳膜电阻标准尺寸（单位：毫米）
body_length = 12.5    # 圆柱主体长度
body_diameter = 5.0   # 圆柱主体直径
lead_length = 25.0    # 引线长度
lead_diameter = 0.9   # 引线直径
...
def add_color_bands(collection, body_length, body_diameter):
    ...
    # 修改后的色环参数（宽度和间距增加一倍）
    band_width = 1.2      # 色环宽度从0.6增加到1.2（增加一倍）
    band_gap = 0.7       # 色环间距从0.35增加到0.7（增加一倍）
...
# 创建集合
collection = bpy.data.collections.new("Resistor_1W_Enhanced")
...
print("1W碳膜电阻3D模型生成完毕！")
print("色环参数调整：")
print("- 色环宽度：0.6mm → 1.2mm（增加一倍）")
print("- 色环间距：0.35mm → 0.7mm（增加一倍）")
