...
# 定义 3/4W 碳膜电阻标准尺寸（单位：毫米）
body_length = 10.0    # 圆柱主体长度
body_diameter = 4.0  # 圆柱主体直径
lead_length = 25.0    # 引线长度
lead_diameter = 0.8   # 引线直径
...
def add_color_bands(collection, body_length, body_diameter):
    """添加色环标记 - 宽度和间距增加一倍"""
    color_map = {
    ...
    # 修改后的色环参数（宽度和间距增加一倍）
    band_width = 1.0      # 色环宽度从0.5增加到1.0（增加一倍）
    band_gap = 0.6       # 色环间距从0.3增加到0.6（增加一倍）
...
# 创建集合
collection = bpy.data.collections.new("Resistor_3_4W_Enhanced")
...
print("3/4W碳膜电阻3D模型生成完毕！")
print("色环参数调整：")
print("- 色环宽度：0.5mm → 1.0mm（增加一倍）")
print("- 色环间距：0.3mm → 0.6mm（增加一倍）")
