...
# 定义 0402 标准尺寸（单位：毫米）
length_mm = 1.0      # 陶瓷本体长度
width_mm = 0.5       # 陶瓷本体宽度
height_mm = 0.35     # 总高度
terminal_length_mm = 0.25  # 焊端延伸出本体的长度
...
# 创建集合
collection = bpy.data.collections.new("Resistor_0402")
bpy.context.scene.collection.children.link(collection)

# 创建材质
metal_mat = create_resistor_material("0402_Metal", (0.92, 0.92, 0.92), metallic=0.9, roughness=0.15)
ceramic_mat = create_resistor_material("0402_Ceramic", (0.75, 0.75, 0.75), metallic=0.0, roughness=0.8)
coating_mat = create_resistor_material("0402_Coating", (0.12, 0.12, 0.12), metallic=0.0, roughness=0.95)
...
mesh_ceramic = bpy.data.meshes.new("0402_Ceramic_Base")
bm_ceramic.to_mesh(mesh_ceramic)
obj_ceramic = bpy.data.objects.new("0402_Ceramic_Base", mesh_ceramic)
...
mesh_left = bpy.data.meshes.new("0402_Left_Terminal")
bm_left.to_mesh(mesh_left)
obj_left = bpy.data.objects.new("0402_Left_Terminal", mesh_left)
...
mesh_right = bpy.data.meshes.new("0402_Right_Terminal")
bm_right.to_mesh(mesh_right)
obj_right = bpy.data.objects.new("0402_Right_Terminal", mesh_right)
...
mesh_coating = bpy.data.meshes.new("0402_Resistor_Coating")
bm_coating.to_mesh(mesh_coating)
obj_coating = bpy.data.objects.new("0402_Resistor_Coating", mesh_coating)
