...
# 定义 2512 标准尺寸（单位：毫米）
length_mm = 6.3      # 陶瓷本体长度
width_mm = 3.2       # 陶瓷本体宽度
height_mm = 0.55     # 总高度
terminal_length_mm = 0.7  # 焊端延伸出本体的长度
...
# 创建集合
collection = bpy.data.collections.new("Resistor_2512")
bpy.context.scene.collection.children.link(collection)

# 创建材质
metal_mat = create_resistor_material("2512_Metal", (0.92, 0.92, 0.92), metallic=0.9, roughness=0.15)
ceramic_mat = create_resistor_material("2512_Ceramic", (0.75, 0.75, 0.75), metallic=0.0, roughness=0.8)
coating_mat = create_resistor_material("2512_Coating", (0.12, 0.12, 0.12), metallic=0.0, roughness=0.95)
...
mesh_ceramic = bpy.data.meshes.new("2512_Ceramic_Base")
bm_ceramic.to_mesh(mesh_ceramic)
obj_ceramic = bpy.data.objects.new("2512_Ceramic_Base", mesh_ceramic)
...
mesh_left = bpy.data.meshes.new("2512_Left_Terminal")
bm_left.to_mesh(mesh_left)
obj_left = bpy.data.objects.new("2512_Left_Terminal", mesh_left)
...
mesh_right = bpy.data.meshes.new("2512_Right_Terminal")
bm_right.to_mesh(mesh_right)
obj_right = bpy.data.objects.new("2512_Right_Terminal", mesh_right)
...
mesh_coating = bpy.data.meshes.new("2512_Resistor_Coating")
bm_coating.to_mesh(mesh_coating)
obj_coating = bpy.data.objects.new("2512_Resistor_Coating", mesh_coating)
