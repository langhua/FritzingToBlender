...
# 定义 1/2W 碳膜电阻标准尺寸（单位：毫米）
body_length = 8.5    # 圆柱主体长度
body_diameter = 3.2  # 圆柱主体直径
lead_length = 25.0   # 引线长度
lead_diameter = 0.7  # 引线直径
...
def add_color_bands(collection, body_length, body_diameter):
    ...
    # 修改后的色环参数（宽度和间距增加一倍）
    band_width = 0.8      # 色环宽度从0.4增加到0.8（增加一倍）
    band_gap = 0.4       # 色环间距从0.2增加到0.4（增加一倍）
...
# 创建集合
collection = bpy.data.collections.new("Resistor_1_2W_Enhanced")
...
print("1/2W碳膜电阻3D模型生成完毕！")
print("色环参数调整：")
print("- 色环宽度：0.4mm → 0.8mm（增加一倍）")
print("- 色环间距：0.2mm → 0.4mm（增加一倍）")
